...
void systemDialog::on_button_create_system_clicked()
{
    ...
    QStringList var_species;
    QList<double> timing;
    timing << this->ui->stepSimBox->value()<<this->ui->endSimBox->value();
    ...
    if(this->ui->list_variable_species->topLevelItemCount()!=0){
        for(int i=0;i<this->ui->list_variable_species->topLevelItemCount();i++){
            var_species << this->ui->list_variable_species->topLevelItem(i)->text(0);
        }
    }
    ...
    if(reaction_list.count()<1){
        ...
        emit createSystemRequestFromDia(name,reaction_list,var_species,timing);
        this->close();
    }
...
void systemDialog::refresh_species_available(){
    ...
    if(this->ui->list_reactions_in_system->topLevelItemCount()!=0){
        for(int i=0;i<this->ui->list_reactions_in_system->topLevelItemCount();i++){
            QStringList tmp = this->ui->list_reactions_in_system->topLevelItem(i)->text(2).split(" ");
            for(int j=0;j<tmp.size();j++){
                if((tmp.at(j)!="Act:") && (tmp.at(j)!="Rep:")){
                    species_available << tmp.at(j);
                }
            }
        }
